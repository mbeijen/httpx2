...
def redirects(request: httpx2.Request) -> httpx2.Response:
    ...
    if request.url.path == "/redirect_301":
        ...
        return httpx2.Response(status_code, headers=headers, content=content)
        ...
        status_code = httpx2.codes.SEE_OTHER if count else httpx2.codes.OK
        if count:
    ...
    if request.url.path == "/redirect_loop":
        ...
        status_code = httpx2.codes.PERMANENT_REDIRECT
...
def test_can_stream_if_no_redirect() -> None:
    ...
    with client.stream("GET", url, follow_redirects=False) as response:
        pass
    assert response.status_code == httpx2.codes.MOVED_PERMANENTLY
    assert response.headers["location"] == "https://example.org/"


class ConsumeBodyTransport(httpx2.MockTransport):
...
def test_cannot_redirect_streaming_body() -> None:
    ...
    with pytest.raises(httpx2.StreamConsumed):
...
def cookie_sessions(request: httpx2.Request) -> httpx2.Response:
    if request.url.path == "/":
