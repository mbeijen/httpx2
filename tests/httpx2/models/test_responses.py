...
def test_raise_for_status() -> None:
    ...
    response = httpx2.Response(101, request=request)
...
def test_iter_raw_doesnt_return_empty_chunks() -> None:
    ...
    response = httpx2.Response(200, content=streaming_body_with_empty_chunks())
...
def test_cannot_read_after_response_closed() -> None:
    ...
    response.close()
    with pytest.raises(httpx2.StreamClosed):
        response.read()
...
def test_generator_with_content_length_header() -> None:
    ...
    response = httpx2.Response(200, content=content(), headers=headers)
...
def test_response_decode_text_using_explicit_encoding() -> None:
    ...
    response = httpx2.Response(200, content=content, default_encoding="cp1252")
